...
{%- set quotesPrinted = false -%} 
  ...
  {# ------------------------------------------------
     DETECTS A NEW "PAGE" (underline from Zotero)
     Resets quotesPrinted for the new page
     ------------------------------------------------ #}
...
<span style="display: inline-block; width: 100%; text-align: right;">[{{ annot.pageLabel }}]({{ annot.desktopURI }})</span>
  ...
  {# ------------------------------------------------
     ...
     Print "##### quotes" once per page if needed
     ------------------------------------------------ #}
  {%- elif annot.color in ["#5fb236", "#ff6666", "#ffd400"] and (annot.annotatedText | length > 0) -%}

    {%- if not quotesPrinted %}
###### quotes
      {%- set quotesPrinted = true -%}
    {%- endif -%}

    {%- if annot.color == "#5fb236" %}
<span style="display: inline-block; width: 100%; text-align: right;"><mark style="background: #3e6e20;">{{ annot.annotatedText }}</mark></span>
    {%- elif annot.color == "#ff6666" %}
<span style="display: inline-block; width: 100%; text-align: right;"><mark style="background: #994141;">{{ annot.annotatedText }}</mark></span>
    {%- elif annot.color == "#ffd400" %}
<span style="display: inline-block; width: 100%; text-align: right;"><mark style="background: #998a26;">{{ annot.annotatedText }}</mark></span>
    {%- endif -%}
